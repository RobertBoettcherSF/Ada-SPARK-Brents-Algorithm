--  Standalone test suite for Brents_Algorithm (SPARK port).
...
begin
   ...
   begin
      ...
      Check (Nat (D.Lambda) = 5, "Detect fills lambda = 5");
      Check (Nat (D.Mu) = 0, "Detect mu stays 0");
      Check (Idx (D.Start_Node) = Null_Index, "Detect start stays null");
   ...
   begin
      ...
      Check (Nat (D.Lambda) = 4, "Detect rho lambda = 4");
      Check (Same_Mu_Lam (R, N), "Brent matches naive on rho");
   ...
   declare
      ...
      D : constant Cycle_Result := Detect (M, 3);
   begin
      Check (Nat (R.Mu) = 2, "wiki mu = 2");
      Check (Nat (R.Lambda) = 3, "wiki lambda = 3");
      Check (Idx (R.Start_Node) = 7, "wiki start = 7");
      Check (Nat (D.Lambda) = 3, "Detect wiki lambda = 3");
      Check (Same_Mu_Lam (R, N), "Brent matches naive on wiki");
   end;
   ...
   Section ("Agreement Brent vs Naive (battery)");
   declare
